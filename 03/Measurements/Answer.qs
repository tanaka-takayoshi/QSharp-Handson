namespace Measurements.References
{
    ...
    operation StatePrep(qs: Qubit[], state: Bool) : Unit {
        let alpha = RandomReal(5) * PI();
        let beta = RandomReal(5) * PI();
        // prep cos(alpha) * |0..0⟩ + sin(alpha) * |1..1⟩
        Ry(2.0 * alpha, qs[0]);
        ...
        if (state) {
            ...
            //ApplyToEachCA(X, qs[0..(Length(qs) / 2 - 1)]);
        }
    }
    ...
    operation IsZeroZeroOneOne(qs: Qubit[]) : Bool {
         ...
         // TODO 2の回答
         //return Measure([PauliZ, PauliZ], qs) == Zero;
    }
